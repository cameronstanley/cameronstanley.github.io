...
In this post, I'm going to go over how to create a Cordova plugin and implement the native APIs for the iOS platform. Cordova allows developers to target multiple mobile platforms using a special web view wrapper to write once and deploy anywhere. In this tutorial, you will implement the JS interface and the underlying iOS implementation for a simple "hello world" plugin.
...
HelloWorld.prototype.welcome = function(success, failure, name) {
	exec(success, failure, 'HelloWorld', 'welcome', [name]);
}
```
